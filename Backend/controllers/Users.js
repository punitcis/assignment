...
export const Register = async(req, res) => {
    ...
    if (password !== confPassword) {
        return res.status(400).json({ msg: "Password and Confirm Password didn't match" });
    }
...
export const Login = async(req, res) => {
    try {
        ...
        res.status(404).json({msg:"Email could not found"});
    }
}
...
export const resetPassword = async (req, res) => {
    ...
    if (!passwordRegex.test(newPassword)) {
        return res.status(400).json({
            msg: "New password must contain at least one lowercase letter, one uppercase letter, one digit, one special character, and be at least 8 characters long"
        });
    }

    // Check if the new password and confirmation password match
    if (newPassword !== confirmPassword) {
        return res.status(400).json({ msg: "New Password and Confirm Password didn't match" });
    }
